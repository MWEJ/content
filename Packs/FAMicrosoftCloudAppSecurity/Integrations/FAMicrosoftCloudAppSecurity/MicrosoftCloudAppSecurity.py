...
class Client(BaseClient):
    ...
    def mark_read(self, url_suffix: str, request_data: dict):
        data = self._http_request(
            method='POST',
            url_suffix=url_suffix,
            json_data=request_data
    ...
    def mark_unread(self, url_suffix: str, request_data: dict):
        data = self._http_request(
            method='POST',
            url_suffix=url_suffix,
            json_data=request_data
...
def mark_read_command(client: Client, args: dict):
    url_suffix = '/alerts/'
    alert_id = args.get('alert_ids')
    custom_filter = args.get('custom_filter')
    arguments = assign_params(**args)
    request_data, url_suffix = build_filter_and_url_to_search_with(url_suffix, custom_filter, arguments, alert_id)
    url_suffix += '/read/'
    alerts_response_data = client.mark_read(url_suffix, request_data)
    list_alert = alerts_response_data.get('data') if 'data' in alerts_response_data.keys() else [alerts_response_data]
    if list_alert:  # organize the output
        alerts = arrange_alerts_by_incident_type(list_alert)
        alerts = arrange_alerts_descriptions(alerts)
...
def mark_unread_command(client: Client, args: dict):
    url_suffix = '/alerts/'
    alert_id = args.get('alert_ids')
    custom_filter = args.get('custom_filter')
    arguments = assign_params(**args)
    request_data, url_suffix = build_filter_and_url_to_search_with(url_suffix, custom_filter, arguments, alert_id)
    url_suffix += '/unread/'
    alerts_response_data = client.mark_unread(url_suffix, request_data)
    list_alert = alerts_response_data.get('data') if 'data' in alerts_response_data.keys() else [alerts_response_data]
    if list_alert:  # organize the output
        alerts = arrange_alerts_by_incident_type(list_alert)
        alerts = arrange_alerts_descriptions(alerts)
...
def main():
    ...
    try:
        ...
        elif demisto.command() == 'microsoft-cas-mark-read':
            return_results(mark_read_command(client, demisto.args()))

        elif demisto.command() == 'microsoft-cas-mark-unread':
            return_results(mark_unread_command(client, demisto.args()))
